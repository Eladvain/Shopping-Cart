...
appSystem::~appSystem()
{
	for (int i = 0; i < AS_arrayOfUsers.size(); i++)
...
{
	int i = 0;
	while (i < AS_arrayOfUsers.size())
	{
		
		if(AS_arrayOfUsers[i]->U_getName().compare(name)==0)
		break;
		i++;
	}
	if (i == AS_arrayOfUsers.size())
	{
	...
	{
		if (AS_arrayOfUsers[i]->U_getPassword().compare(password) == 0)
...
{
	if (AS_arrayOfUsers.size() == AS_arrayOfUsers.capacity())
		AS_arrayOfUsers.reserve(AS_arrayOfUsers.capacity() * 2);
	AS_arrayOfUsers.push_back (ptr);
}
...
user * appSystem::AS_createNewUser(int i)
{
	...
	if (checkValidityByPtr->A_checkValidity(cityS, streetS, number) == true)
	{
		adress ad(cityS, streetS, number);
...
void appSystem::AS_printAllProduct()											const
{
	for (int i = 0; i < AS_arrayOfUsers.size(); i++)
	{
		...
		if (sellerGlasses)
		{
			...
			vector<product*> temp = sellerGlasses->SE_getArrayOfProduct();
		...
		else	
		{
			...
			if (CASGlasses)
			{
				...
				vector<product*> temp = CASGlasses->SE_getArrayOfProduct();
				for (int j = 0; j < sizeofProductArray; j++)
					cout << temp[j] << endl;
			}
		}
	...
	bool exist = false;
		for (int i = 0; i < AS_arrayOfUsers.size(); i++)
		{
			...
			if (sellerGlasses)
			{
				...
				if(temp[i]->P_getname().compare(name)==0)
				{
						cout << temp[i] << endl;
						exist = true;
					}
...
bool appSystem::AS_printAllSellerUserName()													const
{
	...
	bool exist = false;
	vector<user*>::const_iterator itrB = AS_arrayOfUsers.begin();
	vector<user*>::const_iterator itrE = AS_arrayOfUsers.end();

	for (; itrB != itrE; itrB++)
	{
		seller* sellerGlasses = dynamic_cast<seller*>(*itrB);
		if (sellerGlasses)
		{
			exist = true;
			cout <<*itrB << endl;
...
user* appSystem::AS_findSellerForFeedback(char *name)			const // the function will return the adress of the asked seller if he exist
{
	...
	for (; itrB != itrE; itrB++)
	{
		...
		if (sellerGlasses)
		{
			if (sellerGlasses->U_getName().compare( name) == 0)
				return sellerGlasses;
		}
		else 
		{
			consumerAndseller* cAsGlasses = dynamic_cast<consumerAndseller*>(*itrB);
			if (cAsGlasses)
			{
				if (cAsGlasses->U_getName().compare( name) == 0)
					return cAsGlasses;
			}
		}
	}
...
void appSystem::AS_printAllConsumer()				const
{
	...
	for (; itrB != itrE; itrB++)
	{
		consumer* consumerGlasses = dynamic_cast<consumer*>(*itrB);
		if (consumerGlasses!=nullptr)
		{
			cout << *itrB << endl;;
		}
...
bool appSystem::AS_existUser(char*name)										const
{
	...
	for (; itrB != itrE; itrB++)
	{
		if ((*itrB)->U_getName().compare( name) == 0)
			return true;
	}
	return false;
}
	...
	while (stop != true)
	{
		...
		case 2:
		{
			...
			int logicSize = currSeller->SE_getLogicfeedback();
				currSeller->SE_ArrayOfFeedbacks.print();
			_sleep(1000);
			break;
		}
		case 3:
...
void appSystem::AS_OrderMenuForConsumer(consumer *consumerC)
{
	...
	if (answer == 1)
	{
		...
		for (int i = 0; i < AS_arrayOfUsers.size(); i++)
		{
			...
			if (consumerGlasses != nullptr)
			{
				consumerGlasses->C_getShoppingcart()->removeProductFromCart(currentOrder->O_getArrayOfProduct(), currentOrder->O_getArrayOfProduct().size());
			}
		}
...
consumer* appSystem::returnConsumerByUserName(const char*name) const
{
	...
	for (; itrB != itrE; itrB++)
	{
		if ((*itrB)->U_getName().compare( name) == 0)
		{
			consumer* CGlasses = dynamic_cast<consumer*>(*itrB);
			return CGlasses;
		}
	}
	return nullptr;
...
void appSystem::save(ofstream& out)
{
	out << AS_arrayOfUsers.size();
	out << endl;
	vector<user*>::const_iterator itrB = AS_arrayOfUsers.begin();
	vector<user*>::const_iterator itrE = AS_arrayOfUsers.end();
	for (; itrB != itrE; itrB++)
	{
		consumer* cG = dynamic_cast<consumer*>(*itrB);
		seller* sG = dynamic_cast<seller*>(*itrB);
		if (cG != nullptr &&sG != nullptr)
		{
			out << "consumerAndseller ";
			out << *itrB;
		}
		else if (cG != nullptr)
		{
...
appSystem::appSystem(ifstream& in)
{
	AS_arrayOfUsers.reserve(10);
	char username[20];
	char pass[20];
	...
	for(int i=0;i<size;i++)
	{
		...
		in >> street;
		in >> houseNum;
		if (strcmp(name, typeid(consumerAndseller).name() + 6) == 0)
		{
			adress ad(city, street, houseNum);
			(*this) += new consumerAndseller (username, pass, ad);

		}
		else if (strcmp(name, typeid(consumer).name() + 6) == 0)
		{
			adress ad(city, street, houseNum);
			(*this) += new consumer (ad, username, pass); 
		}
		else
		{
			adress ad(city, street, houseNum);
			(*this) += new seller(username,pass,ad);
		}
	}
}

...
public:
	...
	product(const string &name, const  P_type& kind, const float& price,  seller *ptr);//constructor
	...
	void P_setName(const string &name);
	void P_setP_type(const P_type& kind);
	void P_setPrice(const float& price);
	bool P_checkValadity(const string & name, const  P_type& kind, const float& price);// check if the input is valid , we will add product after we checked it .
	const float& P_getprice()							const;
	const int& P_getSerialNumber()						const;
	const string& P_getname()								const;

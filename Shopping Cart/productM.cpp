...
bool product::P_checkValadity(const string& name,  const  P_type& kind, const float& price)
{
	if (name.length() > (MAX_LENGTH_PRODUCT_NAME - 1))
	{
...
void product::P_setName(const  string& name) 
{
	P_name = name;
}
...
//_______________________________________________________________________________________________________________________________
product::product(const string & name,const  P_type& kind, const float& price, seller *ptr)//construcor 
{
	P_setName(name);
	P_setP_type(kind);
	P_setPrice(price);
...
const string& product::P_getname()							const
{
	return P_name;
}
...
{
	int i = 0;
	int sizeOfArray = P_productOwnSeller->SE_getArrayOfProduct().size();
	vector<product*> Parray = P_productOwnSeller->SE_getArrayOfProduct();// we will have the arrayof product belonge to the seller which the product belong to 
	while (i < sizeOfArray ) // as long as we havent reach to the product in the array
	{
		if ((Parray)[i] == prod)
			break;
		i++;
	...
	temp->setSizeOfVector(sizeOfArray - 1);
}
